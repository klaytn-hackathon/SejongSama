...
contract BeggarBetting {
  ...
  mapping(address => uint32) public betterNumWinning;
  uint32 numOfPanhandler;
  uint32 numOfVagabond;
  uint32 numOfTramp;
  uint32 numOfMiddleClass;
  ...
  */
  ...
  /**
    ...
    */
  function placeBet(uint256 _matchId, uint16 _homeTeamScore, uint16 _awayTeamScore, uint64 _bettingPrice) public payable returns (bool) {
  ...
  /**
    ...
    */
  function claimPrizes(uint256 _matchId, uint16 _homeTeamScore, uint16 _awayTeamScore, uint64 _bettingPrice) public returns (bool) {
    uint totalNumBetters = matchBettingInfo[_matchId].length;
    uint32 numOfBetters = 0;
    uint32 numOfWinners = 0;
    ...
    for (uint i = 0; i < totalNumBetters; i++) {
      if (matchBettingInfo[_matchId][i].bettingPrice == _bettingPrice) {
        numOfBetters++;
        if (matchBettingInfo[_matchId][i].homeTeamScore == _homeTeamScore && matchBettingInfo[_matchId][i].awayTeamScore == _awayTeamScore) {
            numOfWinners++;
        }
      }
    }
    // msg.sender is the only winner, gets all the prize and gives a 3% commission to the owner
    if (numOfWinners == 1) {
      commissionToOwner = _bettingPrice * numOfBetters * 3 / 100;
    ...
    // One more winner, divide it equally and gives a 3% commission to the owner
    } else if (numOfWinners > 1) {
      commissionToOwner = ((_bettingPrice * numOfBetters) / numOfWinners) * 3 / 100;
    ...
    _sendCommissionToOwner(commissionToOwner);
    _withdraw();
    _afterClaim(_matchId, _bettingPrice, winningPrize, numOfWinners, numOfBetters);

    return true;
  }

  /**
    * Send 3% commission to the contract owner
    *
    * Send `_commission` to `owner` from the winner's prize
    *
    * @param _commission The commission to be sent to the contract owner
    */
  function _sendCommissionToOwner(uint _commission) private {
  ...
  /**
    ...
    */
  function _afterClaim(uint256 _matchId, uint64 _bettingPrice, uint256 _winningPrize, uint32 _numOfWinners, uint32 _numOfBetters) private {
    ...
    _checkPrivilegeAccomplishment(betterNumWinning[msg.sender]);
  }
 ...
 /**
    ...
    */
  function _checkPrivilegeAccomplishment(uint numWinning) private {
    if (numWinning == 3) {
      numOfPanhandler++;
    }
    if (numWinning == 8) {
  ...
  /**
    ...
    */
  function checkDuplicateMatchId(address _better, uint256 _matchId, uint64 _bettingPrice) public view returns (bool) {
  ...
  /**
    ...
    * @param _bettingPrice The betting price to find the msg.sender's betting info
    */
  function checkPrizeAlreadyReceived(address _better, uint256 _matchId, uint64 _bettingPrice) public view returns (bool) {
    uint numOfBetterBettingInfo = betterBettingInfo[_better].length;
  ...
  /**
    ...
    */
  function getBetterBettingInfo(address _better) public view returns (uint256[], uint16[], uint16[], uint64[]) {
    uint length = betterBettingInfo[_better].length;
    uint256[] memory matchId = new uint256[](length);
    uint16[] memory homeTeamScore = new uint16[](length);
    uint16[] memory awayTeamScore = new uint16[](length);
    uint64[] memory bettingPrice = new uint64[](length);
  ...
  /**
    ...
    * @param _better The better's address to search betting info
    */
  function getBetterBettingInfo2(address _better) public view returns (bool[], bool[], uint256[], uint32[], uint32[]) {
    uint length = betterBettingInfo[_better].length;
    ...
    uint32[] memory numOfWinners = new uint32[](length);
    uint32[] memory numOfBetters = new uint32[](length);
  ...
  /**
    ...
    */
  function getNumOfBettersForMatchAndPrice(uint _matchId, uint64 _bettingPrice) public view returns (uint32) {
  ...
  /**
    ...
    */
  function getBetterNumOfWinnings(address _better) public view returns (uint32) {
  ...
  /**
    ...
    */
  function getInfoPanel() public view returns (uint32, uint32, uint32, uint32) {
